...
/**
 * Created by Ramona on 1/18/2018.
 */
public class DetailCVCompleteChildCameraActivity extends BaseCameraActivity {
    @BindView(R.id.btn_camera)
    ImageView mBtnCamera;
    @BindView(R.id.detail_cv_complete_btn_cancel)
    TextView mTVCancel;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_cv_complete_child);
...
//    }
    ...
    @OnClick(R.id.detail_cv_complete_btn_cancel)
                    ...
                    if (resultApi.getResultInfo().getStatus().equals(VConstant.RESULT_STATUS_OK)) {
                        if (resultApi.getListImage() != null && resultApi.getListImage().size() > 0) {
                            ...
                            if (resultApi.getResultInfo().getMessage() != null && resultApi.getResultInfo().getMessage().length() > 0) {
                                Toast.makeText(DetailCVCompleteChildCameraActivity.this, resultApi.getResultInfo().getMessage(), Toast.LENGTH_SHORT).show();
                            } else
                                Toast.makeText(DetailCVCompleteChildCameraActivity.this, "Người dùng không có ảnh !", Toast.LENGTH_SHORT).show();
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
