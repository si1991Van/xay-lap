...
public class ComplaintDetail3CameraActivity extends BaseCameraActivity {
    ...
    public void getData() {
        ...
        issueWorkItemDTO = (IssueWorkItemDTO) getIntent().getExtras().get("reflect3");
...
//            tvConstruction.setText(issueWorkItemDTO.getConstructionId() + "");
    ...
    private void save() {
        ...
        ApiManager.getInstance().updateIssue(issueWorkItemDTO,
            ...
            public <T> void onResponse(T result) {
                try {
                    ...
                    if (resultInfo.getStatus().equals(VConstant.RESULT_STATUS_OK)) {
                        Toast.makeText(ComplaintDetail3CameraActivity.this, "Cập nhật thành công!", Toast.LENGTH_SHORT).show();
                        App.getInstance().setNeedUpdateIssue(true);
                        finish();
                    } else {
                        Toast.makeText(ComplaintDetail3CameraActivity.this, "Cập nhật thất bại!", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
            ...
            public void onError(int statusCode) {
                Toast.makeText(ComplaintDetail3CameraActivity.this, getString(R.string.server_error), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private boolean checkRadioSave() {
        ...
        if (issueWorkItemDTO.getChangeSysRoleCode()!=null
                &&issueWorkItemDTO.getChangeSysRoleCode() != 1) {
            if (!rb3.isChecked()) {
                isSave = false;
            } else isSave = true;
        }
